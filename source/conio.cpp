...
ConIO* ConIO::Write(DWORDLONG val) 
{
	if (h == 0) return this;
	...
	*--ptr=0;
	if (val == 0) *--ptr='0';
	else while (li.LowPart || li.HighPart) {
		int nibble = li.LowPart&0xF;
		
		li.LowPart = (li.LowPart>>4) | (li.HighPart << 28);
		li.HighPart >>= 4;
...
ConIO* ConIO::WriteLine(DWORDLONG val) 
{
	if (h == 0) return this;
	Write(val);
	WriteLine();
