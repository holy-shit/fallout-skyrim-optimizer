/*
Fallout New Vegas/Skyrim 4GB Helper DLL
...
int Change4GBValue(LPVOID baseaddress)
{
	...
	// Toggle to flag
	pFileHeader->Characteristics = pFileHeader->Characteristics ^ IMAGE_FILE_LARGE_ADDRESS_AWARE;
...
extern "C" __declspec(dllexport) INT WINAPI CompleteInjection(Injection *injection)
{
...
#if FNV4GB
	Console->WriteLine("fnv4gb_helper: successfully injected");
#else
	...
	if (GlobalMemoryStatusEx(&memory_status)) {
		Console->Write("Address Space Size = 0x")->WriteLine(memory_status.ullAvailVirtual);
	}

	if (!injection->szOriginalName[0]) {
		Console->WriteLine("Don't need to reset LAA Bit");
	}
	else {
		Console->WriteLine("Resetting LAA Bit");
		if (Change4GBValue(executable_info.lpBaseOfDll) < 0) {
			Console->WriteLine("Resetting LAA bit failed");

			if (LPVOID(executable) != LPVOID(0x4000000)) {
				Console->WriteLine("Attempting at default addresss 0x400000");

				if (Change4GBValue(LPVOID(0x4000000)) < 0) {
					Console->WriteLine("Second attempt at resetting LAA bit failed");
				}
			} 
		}
...
#if FNV4GB
	Console->WriteLine("fnv4gb_helper: returning from CompleteInjection");
#else
